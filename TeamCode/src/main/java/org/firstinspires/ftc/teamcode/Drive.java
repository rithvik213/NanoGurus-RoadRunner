...
public class Drive {
    ...
    /**
     *
     * @param angle - angle the robot wants to turn to
     * @param direction - which direction to turn to (true = right, false = left)
     */
    ...
    public void turnIMUOneSide(double angle, double speed, boolean direction) {
        ...
        opMode.telemetry.addData("Angle when target calculated: " ,Math.abs((int)initialPos));
        ...
        while(opMode.opModeIsActive() && ((Math.abs(target - Math.abs(currentPos))) > 2)) {
            ...
            error = Math.abs((currentPos - target));

            AngleWrap(error);

            double power = Range.clip(((error/100.0) + i), .3,.7);
